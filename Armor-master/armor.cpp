...
armor::armor(const LED_Stick& L1, const LED_Stick& L2,Mat& img)
{
    ...
    Led_stick[1] = L2;
    //angle diff
    circle(img,L1.rect.center,3,Scalar(255,0,0),-1);
    circle(img,L2.rect.center,3,Scalar(255,0,0),-1);
...
{
    if(Led_stick[0].rect.size.height * 0.7 < Led_stick[1].rect.size.height
            && Led_stick[0].rect.size.height * 1.4 > Led_stick[1].rect.size.height)
    {
        ...
        {
            if(h_max * 5 > rect.width && h_max < rect.width)
                return true;
        }
    }
    return false;
